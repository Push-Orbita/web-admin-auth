...
// import { MarcaApi } from "../../../services/actions/actions.service"
...
const FormTypeMarca: React.FC<FormTypeActionsProps> = ({ refetch }) => {
    ...
    const postActions = UseQueryMutation({
        requestFn: MarcaApi.postActions,
        options: {
            onError() {
                toast.error(t(lang.MarcaType.messages.createdSuccess))
            },
            onSuccess: () => {
    ...
    const patchActions = UseQueryMutation({
        requestFn: MarcaApi.patchActions,
        options: {
            onError() {
